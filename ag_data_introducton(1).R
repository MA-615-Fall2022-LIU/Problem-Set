...
# Gradescope
chemical <- filter(strawb, Domain == "CHEMICAL, FUNGICIDE"| Domain =="CHEMICAL, HERBICIDE"| Domain == "CHEMICAL, INSECTICIDE"| Domain == "CHEMICAL, OTHER"| Domain == "FERTILIZER")
unique(chemical$`Domain Category`)
grep('TOTAL', chemical$`Domain Category`,ignore.case = T)
chemical_ca <- filter(chemical, State == 'CALIFORNIA')
unique(chemical_ca$`Domain Category`)
grep('TOTAL', chemical_ca$`Domain Category`,ignore.case = T)
chemical_fl <- filter(chemical, State == 'FLORIDA')
unique(chemical_fl$`Domain Category`)
grep('TOTAL', chemical_fl$`Domain Category`,ignore.case = T)

sales_og <- filter(strawb, State == 'CALIFORNIA'& Year == 2016 & Domain == 'ORGANIC STATUS')
as.integer(sales_og$Value[1])*1.96*0.137

sales_nog <- filter(strawb, State == 'CALIFORNIA'& Year == 2016& Domain != 'ORGANIC STATUS')
sales_nog_new <- filter(sales_nog, Value != '(NA)'& Value != '(D)'& Domain != 'TOTAL')

unique(sales_nog_new$`Domain Category`)
